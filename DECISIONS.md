...
- **R5 — Vector store.** Phase 2 vector store is a self-managed on-prem OpenSearch cluster. No managed-cloud vector store: rm_knowledge contains customer data. *Applied: SA, TI.*
...
- **SA5 — RAG data classification.** fraud_knowledge: internal-sensitive; compliance_knowledge: public regulatory + internal policy; rm_knowledge: customer data — excluded from all cloud backup paths; on-prem (MinIO) snapshots only.
...
- **EB8 — Build tracker.** Section 18 tracker retired from the locked document; living copy is BUILD_TRACKER.md in this repo.
